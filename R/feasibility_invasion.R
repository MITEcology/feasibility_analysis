...
#' @param nsample Numeric, number of sampling points, default to 5000
#'
...
feasibility_invasion <- function(matA, inv, nsample = 5000) {
  S <- nrow(matA)
  res <- c(1:S)[-inv]

  # sample r uniformly from the unit sphere
  sample_r <- runif_on_sphere(n = nsample, d = S, r = 1)
